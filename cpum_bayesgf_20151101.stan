// g-formula for CPUM data, correcting for Berkson measurement error
data {
...
parameters{
    ...
    row_vector<lower=0>[obscomplete] berkson;
...
//      g0 ~ normal(0, 100); // uniform prior if commented out
    ...
    //hyperpriors for measurement error
    // can I come up with a useful prior here?
    z0 ~ normal(0, 1); //essentially prior of intercept for model relating observed vs. actual exposure
    sigma2_z ~ inv_gamma(10, 1); //scale parameter is switched in stan vs. sas (stan scale parameter = iscale parameter in sas), same parameterization as in R


        ////////
        //measurement model
        ////////
        //prior information: coefficient of variation (sd/mean) about 50%;
        log(berkson) ~ normal(z0, sigma2_z); 
        increment_log_prob(-(log(berkson))); // see stan 2.8.0 ref manual sec. 15.3
{
    //true exposure variables
    row_vector[obscomplete] truecumwlm2lag;
    row_vector[obscomplete] truecumwlm2lagcen;
    row_vector[obscomplete] truewlm;

    //cumulative true exposure, assumed multiplicative measurement error
    for (n in 1:obscomplete){
        truewlm[n] <- wlm[n]*berkson[n];    
        if(n<=2 || id[n] != id[n-2]){
            truecumwlm2lag[n] <- 0;
            truecumwlm2lagcen[n] <- 0;
        }
        else{
            truecumwlm2lag[n] <- truecumwlm2lag[n-1] + truewlm[n-2];
            truecumwlm2lagcen[n] <- (truecumwlm2lag[n] - meancumwlm2lag)/sdcumwlm2lag;
        }
    }//n in obscomplete
    ...
    for (n in 1:obscomplete){
       ///////// 
       // true exposure [based on richardson and gilks 1993, stram et al
       // row_vector[obs] truewlm
       // row_vector[obs] truecumwlm2lag
       // row_vector[obs] truecumwlm2lagcen
       ///////// 
        ...
        leftwork[n] ~ bernoulli_logit(
            g0 + g[1]*BL_cumwlmcen[n] + g[2]*BL_cumyrsexpcen[n] + g[3]*cohort_1[n] + g[4]*cumyrsexpcen[n] + 
            g[5]*truecumwlm2lagcen[n] + g[6]*ageoutcen[n] + 
            g[7]*ageoutcen[n]*ageoutcen[n]  +
            g[8]*dateoutcen[n] + g[9]*dateoutcen[n]*dateoutcen[n]
       );
        } else increment_log_prob(0);
        
        ////////
        // exposure model (does this need the corrected exposure?)
        ////////
        if(atwork[n]==1 && wlm[n]>0){
        log(truewlm[n]/1000) ~ normal(
            a0 + a[1]*BL_cumwlmcen[n] + a[2]*BL_cumyrsexpcen[n] + a[3]*cohort_1[n] + a[4]*cumyrsexpcen[n] + a[5]*truecumwlm2lagcen[n] + 
            a[6]*ageoutcen[n] + 
            a[7]*ageoutcen[n]*ageoutcen[n] +
            a[8]*dateoutcen[n] + a[9]*dateoutcen[n]*dateoutcen[n]
            , sigma2
            );
            increment_log_prob(-(log(truewlm[n]/1000)));
        } else increment_log_prob(0);
        ...
        d_lc ~ bernoulli_logit(
            b0 + b[1]*BL_cumwlmcen + b[2]*BL_cumyrsexpcen + b[3]*cohort_1 + b[4]*cumyrsexpcen + b[5]*atwork2lag + 
            b[6]*truecumwlm2lagcen + b[7]*ageoutcen + b[8]*ageoutcen .* ageoutcen + 
            b[9]*ageoutcen .* ageoutcen .* ageoutcen + 
            b[10]*ageoutcen .* ageoutcen .* ageoutcen .* ageoutcen + 
            b[11]*atwork .* truecumwlm2lagcen + 
            b[12]* cumyrsexpcen .* cumyrsexpcen +
            b[13]*dateoutcen + b[14]*dateoutcen .* dateoutcen
        );
        ////////
        // all other cause mortality model
        ////////
        d_nonlc ~ bernoulli_logit(
            c0 + c[1]*BL_cumwlmcen + c[2]*BL_cumyrsexpcen + c[3]*cohort_1 + c[4]*cumyrsexpcen + c[5]*atwork2lag + 
            c[6]*truecumwlm2lagcen + c[7]*ageoutcen + c[8]*ageoutcen .* ageoutcen + 
            c[9]*ageoutcen .* ageoutcen .* ageoutcen  + 
            c[10]*ageoutcen .* ageoutcen .* ageoutcen .* ageoutcen + 
            c[11]*atwork .* truecumwlm2lagcen + 
            c[12]* cumyrsexpcen .* cumyrsexpcen +
            c[13]*dateoutcen + c[14]*dateoutcen .* dateoutcen
        );
   ...
   //using likelihood to generate potential expected covariate, outcomes
    for (n in 1:obs){
        ...
        leftworkf[n] <- bernoulli_rng(inv_logit(
            g0 + g[1]*BL_cumwlm_fullcen[n] + g[2]*BL_cumyrsexp_fullcen[n] + g[3]*cohort_1_full[n] + g[4]*cumlcen[n] + 
            g[5]*cumx2lagcen[n] + g[6]*time_fullcen[n] + 
        ...
        if(lhat[n]==1){
            ...
            a0 + a[1]*BL_cumwlm_fullcen[n] + a[2]*BL_cumyrsexp_fullcen[n] + a[3]*cohort_1_full[n] + a[4]*cumlcen[n] + a[5]*cumx2lagcen[n] +
            a[6]*time_fullcen[n] + 
    ...
    h_lcn[n] <- inv_logit(
        b0 + b[1]*BL_cumwlm_fullcen[n] + b[2]*BL_cumyrsexp_fullcen[n] + b[3]*cohort_1_full[n] + b[4]*cumlcen[n] + b[5]*lhat2lag[n] + 
        b[6]*cumx2lagcen[n] + b[7]*time_fullcen[n] + b[8]*time_fullcen[n]*time_fullcen[n] +
    ...
    ////////
    h_nlcn[n] <- inv_logit(
        c0 + c[1]*BL_cumwlm_fullcen[n] + c[2]*BL_cumyrsexp_fullcen[n] + c[3]*cohort_1_full[n] + c[4]*cumlcen[n] + c[5]*lhat2lag[n] + 
        c[6]*cumx2lagcen[n] + c[7]*time_fullcen[n] + c[8]*time_fullcen[n]*time_fullcen[n] + 
